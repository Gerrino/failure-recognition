...
def binary_search(fun_x: Callable, xi_range: Tuple[float, float], y_target: float, epsilon: float, max_iter: int, xi_range_0: Tuple[float, float] = None, pos_slope: bool = True):
    '''
...
def find_signal_peaks(t: np_typing.ArrayLike, signal: np_typing.ArrayLike, num_peaks: int, mode: FindPeaksMode, x_0: dict = None, xi_range_0: Tuple[float, float] = None, max_iterations: int = 20) -> Tuple[np_typing.ArrayLike, np_typing.ArrayLike]:
    ...
    opt = binary_search(lambda p: len(find_peaks(signal, **(x_0.update({mode.value["name"]:p}) or x_0))[0]), mode.value["range"], num_peaks, 0, max_iterations, xi_range_0, mode.value["pos_slope"])
