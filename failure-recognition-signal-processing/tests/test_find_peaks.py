...
from failure_recognition.signal_processing.signal_helper import FindPeaksMode, find_signal_peaks, get_fft
...
class FindPeaksTest(unittest.TestCase):
    ...
    def test_find_all_peaks(self):
        ...
        for mode in FindPeaksMode:
            x_peaks, y_peaks = find_signal_peaks(self.xf, self.yyf, 100, mode, x_0={"prominence":0.01, "threshold": 2, "distance": 1}, max_iterations=500)
    ...
    def test_sinc_peaks(self):
        ...
        for mode in FindPeaksMode:
            x_peaks, y_peaks = find_signal_peaks(self.t, sinc_10, 55, mode, max_iterations=500)            
            self.assertTrue(len(x_peaks) == len(y_peaks))
