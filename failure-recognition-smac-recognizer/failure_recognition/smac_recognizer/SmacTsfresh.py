...
def registerDecisionTreeHyperparams(cs, incumbent):
    print("\nregister\n numTrees"  + str(incumbent["num_trees"] if "num_trees" in incumbent else 10))
    num_trees = UniformIntegerHyperparameter("num_trees", 10, 50, default_value=incumbent["num_trees"] if "num_trees" in incumbent else 10)
    min_weight_frac_leaf = UniformFloatHyperparameter("min_weight_frac_leaf", 0.0, 0.5, default_value=incumbent["min_weight_frac_leaf"] if "min_weight_frac_leaf" in incumbent else 0.0)
    criterion = CategoricalHyperparameter("criterion", ["mse", "mae"], default_value=incumbent["criterion"] if "criterion" in incumbent else "mse")
    min_samples_to_split = UniformIntegerHyperparameter("min_samples_to_split", 2, 20, default_value=incumbent["min_samples_to_split"] if "min_samples_to_split" in incumbent else 2)
    min_samples_in_leaf = UniformIntegerHyperparameter("min_samples_in_leaf", 1, 20, default_value=incumbent["min_samples_in_leaf"] if "min_samples_in_leaf" in incumbent else 1)
    max_leaf_nodes = UniformIntegerHyperparameter("max_leaf_nodes", 10, 1000, default_value=incumbent["max_leaf_nodes"] if "max_leaf_nodes" in incumbent else 100)
    bootstrapping = CategoricalHyperparameter("do_bootstrapping", ["False", "True"], default_value=incumbent["do_bootstrapping"] if "do_bootstrapping" in incumbent else "True")
    cs.add_hyperparameters([num_trees, min_weight_frac_leaf, criterion,
                        min_samples_to_split, min_samples_in_leaf, max_leaf_nodes, bootstrapping])
    print(str(cs))
    
def CreateSmac(cs, timeseries, testSettings, y, featureContainer, scenarioDict, seed, window_size_ratio)->SMAC4HPO:
    rf_from_cfg = lambda cfg, seed: (
        RandomForestFromCFG.rf_from_cfg_extended(cfg, seed,  timeseries, testSettings, y, featureContainer, window_size_ratio))
    scenarioDict["cs"] = cs
...
def LoadFeatureContainer(path_feat_list):    
    ...
    print(f"   clear{', '.join(f.Name for f in featureContainer.FeatureList if f.Enabled and len(f.InputParameters) == 0)}")
    print()
...
def Smac_tsfresh_optimize(timeseries, testSettings, y, featureContainer, scenarioDict, seed, window_size_ratio):
    ...
    for sensor in sensors:
        for f in filter(lambda f: f.Enabled and len(f.InputParameters) > 0, featureContainer.FeatureList):#drop existing default values of enabled features
            print("will it drop")    
            drop_name = None
    ...
    smac = CreateSmac(cs, timeseries, testSettings, y, featureContainer, scenarioDict, seed, window_size_ratio)
    ...
    newHistoryRow = pd.DataFrame({'datetime': [datetime.datetime.now()], 'timespan': [datetime.datetime.now() - dateTimeOptStart], 'action': [f"Optimize {nameOptFeat}"], 'orig-value': [def_value], 'opt-value': [inc_value]})
...
def smac_tsfresh_window_opt(timeseries, testSettings, y, pathDict, scenarioDict, window_size, overlap, seed, window_size_ratio):
    ...
    tot_it = round(min(1, np.ceil(enabled_feature_count / window_size)) + max(0, np.ceil((enabled_feature_count - window_size) / incr)))
    it = 1
    incumbent = {}
    while window_start_pntr < enabled_feature_count:
        ...
        print('|'.join('█' if f.Enabled else '░' for f in enabledFeatures) + f" -> {', '.join(f.Name for f in featureContainer.FeatureList if f.Enabled)}")     
        print()                
        window_start_pntr += incr
        it += 1
        incumbent.update(Smac_tsfresh_optimize(timeseries, testSettings, y, featureContainer, scenarioDict, seed, window_size_ratio))
